        ...
        {toasts.map((toast) => {
          ...
          return (
            <Toast.Root
              key={toast.id}
              className={cn(
                "absolute z-[calc(9999-var(--toast-index))] h-(--toast-calc-height) w-full select-none rounded-lg border border-lemon/20 bg-[color-mix(in_srgb,var(--popover),var(--color-black)_calc(1%*max(0,var(--toast-index,0))))] not-dark:bg-clip-padding text-popover-foreground shadow-lg/5 shadow-[0_0_24px_rgba(227,255,0,0.08)] [transition:transform_.5s_cubic-bezier(.22,1,.36,1),opacity_.5s,height_.15s,background-color_.5s] before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-lg)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] data-expanded:bg-popover dark:bg-[color-mix(in_srgb,var(--popover),var(--color-black)_calc(6%*max(0,var(--toast-index,0))))] dark:data-expanded:bg-popover dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
              ...
              <Toast.Content
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex flex-col gap-0.5">
                    <Toast.Title className="font-tech font-bold uppercase tracking-[0.14em] text-[11px]" data-slot="toast-title" />
                    <Toast.Description className="text-muted-foreground text-xs tracking-wide" data-slot="toast-description" />
                  </div>
                </div>
                {toast.actionProps && (
